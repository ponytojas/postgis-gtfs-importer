...
First, the GTFS data is downloaded to, unzipped into and [tidied](https://github.com/patrickbr/gtfsclean) within `/tmp/gtfs`; You can specify a custom path using `$GTFS_TMP_DIR`.
...
_Note:_ `postgis-gtfs-importer` requires a database user/role that is [allowed](https://www.postgresql.org/docs/14/sql-alterrole.html) to create new databases (`CREATEDB` privilege).
...
If you run with `GTFSCLEANN_BEFORE_IMPORT=false`, gtfsclean will not be used.
...
A new major version of `postgis-gtfs-importer` _does not_ clean up imports done by the previous (major) versions.
